...
/* String declarations */
static std::string const __str_literal_1("Value: %0b", 10u);
...
{
  init_symbol(&symbols[0u], "mod", SYM_MODULE, &INST_mod);
  init_symbol(&symbols[1u], "RL_receive", SYM_RULE);
  init_symbol(&symbols[2u], "RL_send", SYM_RULE);
  init_symbol(&symbols[3u], "sent", SYM_MODULE, &INST_sent);
}


/* Rule actions */

void MOD_mkTb::RL_send()
{
  INST_mod.METH_put(4294967295u);
...
{
  tUInt32 DEF_v__h228;
  tUInt32 DEF_AVMeth_mod_get;
  DEF_AVMeth_mod_get = INST_mod.METH_get();
  DEF_v__h228 = DEF_AVMeth_mod_get;
  if (!(PORT_RST_N == (tUInt8)0u))
  {
    dollar_display(sim_hdl, this, "s,32", &__str_literal_1, DEF_v__h228);
...
void MOD_mkTb::reset_RST_N(tUInt8 ARG_rst_in)
{
  ...
  INST_mod.reset_RST_N(ARG_rst_in);
}
...
{
  INST_mod.dump_VCD(dt, levels, backing.INST_mod);
}
